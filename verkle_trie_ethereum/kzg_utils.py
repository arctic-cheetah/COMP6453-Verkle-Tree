...
class KzgUtils:
    ...
    def evaluate_polynomial_in_evaluation_form(self, f, z):
        ...
        r = (
            r
            * (pow(z, self.WIDTH, self.MODULUS) - 1)
            * self.inverse_width
            % self.MODULUS
        )
    ...
    def check_kzg_proof(self, C, z, y, pi):
        ...
        """
        pairing = blst.PT(
            blst.G2().to_affine(), C.dup().add(blst.G1().mult(y).neg()).to_affine()
        )
        pairing.mul(
            blst.PT(
                self.SETUP["g2"][1].dup().add(blst.G2().mult(z).neg()).to_affine(),
                pi.dup().neg().to_affine(),
            )
        )
    ...
    def compute_commitment_lagrange(self, values):
        ...
        """
        commitment = pippenger.pippenger_simple(
            [self.SETUP["g1_lagrange"][i] for i in values.keys()], values.values()
        )
        return commitment
