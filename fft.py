...
def _fft(vals, modulus, roots_of_unity):
    ...
    for i, (x, y) in enumerate(zip(L, R)):
        y_times_root = (
            y.dup().mult(roots_of_unity[i])
            if isinstance(vals[0], blst.P1)
            else y * roots_of_unity[i]
        )
        o[i] = (
            x.dup().add(y_times_root)
            if isinstance(vals[0], blst.P1)
            else (x + y_times_root) % modulus
        )
        o[i + len(L)] = (
            x.dup().add(y_times_root.neg())
            if isinstance(vals[0], blst.P1)
            else (x - y_times_root) % modulus
        )
    return o
...
def fft(vals, modulus, root_of_unity, inv=False):
    ...
    if inv:
        ...
        if isinstance(vals[0], blst.P1):
            return [x.dup().mult(invlen) for x in _fft(vals, modulus, rootz[:0:-1])]
        else:
