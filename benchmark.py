...
def _gen_and_verify(vk, tree, keys, values, k: int):
    ...
    with Timer() as t_verify:
        ok = tree.check_verkle_proof(
            tree.root.commitment.compress(), sel, [values[k] for k in sel], proof
        )
                ...
                for use_naive in [False, True] if args.compare_naive else [False]:
                    ...
                    if use_naive:
                        if not hasattr(pip, "lincomb_naive"):
                            ...
                            def _g1_zero():
                                try:
                                    return blst.G1().mult(0)
                                except AttributeError:
                                    return blst.P1().mult(0)
                    ...
                    try:
                        for rep in range(args.repeats):
                            ...
                            set_seed(seed)
                            print(
                                f"[run] wb={wb} n={n} k={k} mode={'naive' if use_naive else 'pippenger'} rep={rep + 1}/{args.repeats}"
                            )
                            tree, keys, values, t_build, t_root = _build_tree(vk, n)
                            proof, psize, t_proof, t_verify, ok = _gen_and_verify(
                                vk, tree, keys, values, k
                            )
                            row = {
                                "width_bits": wb,
                                "n": n,
                                ...
                                "build_s": t_build.sec,
                                "build_mem_kb": t_build.mem_kb,
                                "root_s": t_root.sec,
                                "root_mem_kb": t_root.mem_kb,
                                "proof_s": t_proof.sec,
                                "proof_mem_kb": t_proof.mem_kb,
                                "verify_s": t_verify.sec,
                                "verify_mem_kb": t_verify.mem_kb,
                                "proof_size_bytes": psize,
                                "verify_ok": bool(ok),
                            }
                            print(
                                "   times(s): build={:.3f} root={:.3f} proof={:.3f} verify={:.3f} | size={}B | ok={}".format(
                                    row["build_s"],
                                    row["root_s"],
                                    row["proof_s"],
                                    row["verify_s"],
...
def cmd_merkle(args):
    ...
    for k in args.k:
        ...
        for rep in range(args.repeats):
            ...
            layers = _merkle_build(keys, vals)
            t1 = time.perf_counter()
            idxs = random.sample(range(args.n), min(k, args.n))
            t2 = time.perf_counter()
            proofs = [_merkle_prove(layers, i) for i in idxs]
            t3 = time.perf_counter()
            root = layers[-1][0]
            leaves = [_sha(keys[i] + vals[i]) for i in idxs]
            t4 = time.perf_counter()
            oks = [
                _merkle_verify(root, leaf, pf, i)
                for leaf, pf, i in zip(leaves, proofs, idxs)
            ]
            ...
            reps.append(
                {
                    ...
                    "prove_s": t3 - t2,
                    "verify_s": t5 - t4,
                    "avg_proof_size": sum(len(x) * 32 for x in proofs)
                    / max(1, len(proofs)),
                    "ok": all(oks),
                }
            )
        agg = {
            fld: median(r[fld] for r in reps)
            for fld in ["build_s", "prove_s", "verify_s", "avg_proof_size"]
        }
...
def cmd_plot(args):
    ...
    def plot_time_vs_k(metric, out_path):
        ...
        if not subset:
            print(f"[warn] no rows for width_bits={wb}")
            return
        plt.figure()
        groups = {}
        for r in subset:
...
def cmd_plot_wb(args):
    ...
    rows = [
        r
        for r in rows
        if str(r.get("mode", "pippenger")) == "pippenger"
        and int(float(r.get("n", -1))) == args.n
    ]
    if not rows:
        print(f"[warn] no rows for n={args.n} in {args.verkle_csv}")
        return
    ...
    def _parse_lim(s):
        ...
        try:
            ...
            print(f"[warn] bad ylim spec: {s} (expect 'min,max')")
            return None
...
def cmd_plot_n(args):
    ...
    if not rows:
        print(
            f"[warn] no rows for wb={args.width_bits}, k={args.k} in {args.verkle_csv}"
        )
        return
    rows.sort(key=lambda r: int(float(r["n"])))
    xs = np.array([int(float(r["n"])) for r in rows], dtype=float)
    for metric, fname in [
        ("proof_s", "time_vs_n_proof.png"),
        ("verify_s", "time_vs_n_verify.png"),
    ]:
...
def cmd_plot_size(args):
    ...
    if not rows:
        print(f"[warn] no rows for width_bits={args.width_bits} in {args.verkle_csv}")
        return
    groups = {}
    for r in rows:
        nval = int(float(r["n"]))
        groups.setdefault(nval, []).append(r)
    import matplotlib.pyplot as plt
...
def cmd_plot_total(args):
    ...
    for wb in args.width_bits:
        ...
        for r in v_rows:
            ...
            if cs is None:  # 兼容旧CSV
                cs = sum(
                    to_float(r.get(f), 0.0)
                    for f in ("build_s", "root_s", "proof_s", "verify_s")
                )
    ...
    if args.merkle_csv:
        ...
        for r in m_rows:
            ...
            if cs is None:
                cs = sum(
                    to_float(r.get(f), 0.0) for f in ("build_s", "prove_s", "verify_s")
                )
...
def cmd_compare_k(args):
    ...
    def plot_one(xa, ya, xb, yb, ylabel, fname):
        ...
        plt.figure()
        if xa:
            plt.plot(xa, ya, marker="o", label=args.label_a)
        if xb:
            plt.plot(xb, yb, marker="x", linestyle="--", label=args.label_b)
        plt.xscale("log")
        plt.xlabel("k (keys per proof)")
        plt.ylabel(ylabel)
        plt.legend()
...
def _summarize(bench_csv, merkle_csv=None, width_bits_focus=8, n_focus=16384):
    ...
    if not rows:
        print("[summary] no rows found.")
        return
    # focus subset
    wb_rows = [
        r
        for r in rows
        if int(float(r.get("width_bits", -1))) == width_bits_focus
        and str(r.get("mode", "pippenger")) == "pippenger"
    ]
    ...
    def _wb_delta(k):
        ...
        for r in rows:
            if (
                int(float(r.get("k", -1))) == k
                and str(r.get("mode", "pippenger")) == "pippenger"
                and int(float(r.get("n", -1))) == n_focus
            ):
    ...
    if s100 is not None:
        print(f"Proof size @k=100  ~ {s100:.1f} KB")
    if s1k is not None:
        print(f"Proof size @k=1000 ~ {s1k:.1f} KB")
    if s5k is not None:
        print(f"Proof size @k=5000 ~ {s5k:.1f} KB")
    for tag, d in [("k=1", d1), ("k=100", d100), ("k=1000", d1k)]:
        if d:
            mn, mx, delta, table = d
            print(
                f"wb effect {tag}: spread ≈ {delta:.1f}%  (seconds per proof: {table})"
            )
    if merkle_csv and os.path.exists(merkle_csv):
        ...
        if mrows:
            ...
            ms = [
                (int(float(r["k"])), float(r["prove_s"]))
                for r in mrows
                if int(float(r["n"])) == n_focus
            ]
            if ms:
                ms.sort()
...
def cmd_suite(args):
    ...
    cmd_plot(
        _ns(
            verkle_csv=bench_csv,
            merkle_csv=merkle_csv,
            width_bits=args.wb_focus,
    ...
    if args.eth_csv and os.path.exists(args.eth_csv):
        ...
        cmd_compare_k(
            _ns(
                verkle_csv_a=bench_csv,
                verkle_csv_b=args.eth_csv,
                label_a="ours",
                label_b="eth",
                width_bits=args.wb_focus,
                n=args.n_focus,
    ...
    _summarize(
        bench_csv,
        merkle_csv=merkle_csv,
        width_bits_focus=args.wb_focus,
...
def main():
    ...
    if "--total" in sys.argv:
        sys.argv = [
            ...
            "16",
            "--out",
            "results/plot_total_multi_wb.png",
        ]

    ap = argparse.ArgumentParser(
        description="Verkle/Merkle benchmarking & plotting (all-in-one)"
    )
    sub = ap.add_subparsers(dest="cmd", required=True)

    ap_v = sub.add_parser("verkle", help="run verkle benchmarks")
    ap_v.add_argument(
        "--width-bits", nargs="+", type=int, default=[8], help="e.g., 4 8 16"
    )
    ap_v.add_argument(
        "--n", nargs="+", type=int, default=[2**12, 2**14], help="num initial keys"
    )
    ap_v.add_argument(
        "--k", nargs="+", type=int, default=[1, 100, 1000, 5000], help="keys per proof"
    )
    ...
    ap_v.add_argument(
        "--compare-naive",
        action="store_true",
        help="also run naive lincomb baseline (slow)",
    )
    ...
    ap_p.add_argument(
        "--verkle-csv", default=os.path.join(ROOT, "results", "bench.csv")
    )
    ap_p.add_argument(
        "--merkle-csv", default=os.path.join(ROOT, "results", "merkle.csv")
    )
    ap_p.add_argument("--width-bits", type=int, default=8)
    ap_p.add_argument(
        "--out-proof", default=os.path.join(ROOT, "results", "plot_proof_time.png")
    )
    ap_p.add_argument(
        "--out-verify", default=os.path.join(ROOT, "results", "plot_verify_time.png")
    )
    ap_p.set_defaults(func=cmd_plot)

    ap_wb = sub.add_parser(
        "plot-wb", help="plot Verkle curves across width_bits at fixed n"
    )
    ap_wb.add_argument(
        "--verkle-csv", default=os.path.join(ROOT, "results", "bench.csv")
    )
    ...
    ap_wb.add_argument(
        "--out-proof", default=os.path.join(ROOT, "results", "plot_wb_proof.png")
    )
    ap_wb.add_argument(
        "--out-verify", default=os.path.join(ROOT, "results", "plot_wb_verify.png")
    )
    ap_wb.add_argument(
        "--out-size", default=os.path.join(ROOT, "results", "plot_wb_size.png")
    )
    ap_wb.add_argument(
        "--ylim-proof", default="", help="y-limits for proof plot, e.g. 0.3,1.2"
    )
    ap_wb.add_argument(
        "--ylim-verify", default="", help="y-limits for verify plot, e.g. 0.02,0.12"
    )
    ...
    ap_sz = sub.add_parser(
        "plot-size", help="plot proof size (KB) vs k at fixed width_bits"
    )
    ap_sz.add_argument(
        "--verkle-csv", default=os.path.join(ROOT, "results", "bench.csv")
    )
    ...
    ap_sz.add_argument(
        "--out", default=os.path.join(ROOT, "results", "plot_size_vs_k.png")
    )
    ap_sz.set_defaults(func=cmd_plot_size)

    ap_ck = sub.add_parser(
        "compare-k", help="compare time vs k across two CSVs at fixed (width_bits, n)"
    )
    ...
    ap_s.add_argument(
        "--full", action="store_true", help="full scale (slower). default: quick mode"
    )
    ap_s.add_argument(
        "--run-merkle",
        action="store_true",
        help="also run Merkle baseline for overlay in Exp A",
    )
    ap_s.add_argument(
        "--eth-csv",
        default="",
        help="optional: path to ETH implementation bench.csv for Exp E",
    )
    ap_s.add_argument(
        "--wb-focus", type=int, default=8, help="focus width_bits for plots A/B/D/E"
    )
    ap_s.add_argument(
        "--n-focus", type=int, default=16384, help="focus n for plots C/E & summary"
    )
    ap_s.add_argument("--k-focus", type=int, default=1000, help="focus k for plot-n")
    ap_s.set_defaults(func=cmd_suite)

    p = sub.add_parser(
        "plot-total", help="Plot proof/verify time vs k (multi width_bits)"
    )
