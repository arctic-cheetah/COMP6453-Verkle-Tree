...
def update_verkle_node(root, key, value):
        ...
        if index in current_node:
            if current_node[index]["node_type"] == "leaf":
                ...
                if current_node[index]["key"] == key:
                    ...
                    value_change = (
                        MODULUS
                        + int.from_bytes(new_node["hash"], "little")
                    ...
                    value_change = (
                        MODULUS
                        + int.from_bytes(new_inner_node["hash"], "little")
                        - int.from_bytes(old_node["hash"], "little")
                    ) % MODULUS
                    break
    ...
    for index, node in reversed(path):
        ...
        value_change = (
            MODULUS
            + int.from_bytes(new_hash, "little")
            - int.from_bytes(old_hash, "little")
        ) % MODULUS
...
def delete_verkle_node(root, key):
    ...
    while True:
        ...
        if current_node[index]["node_type"] == "leaf":
            ...
            value_change = (
                MODULUS - int.from_bytes(deleted_node["hash"], "little")
            ) % MODULUS
            break
        current_node = current_node[index]
    ...
    for index, node in reversed(path):
        ...
        if only_child != None and only_child["node_type"] == "leaf" and node != root:
            ...
            value_change = (
                MODULUS
                + int.from_bytes(only_child["hash"], "little")
                - int.from_bytes(node["hash"], "little")
            ) % MODULUS
        else:
            node["commitment"].add(SETUP["g1_lagrange"][index].dup().mult(value_change))
            old_hash = node["hash"]
...
def log_time_if_eligible(string, width, eligible):
    if eligible:
        print(
            string
            + " " * max(1, width - len(string))
            + "{0:7.3f} s".format(time() - lasttime[0]),
            file=sys.stderr,
        )
        lasttime[0] = time()
...
def make_kzg_multiproof(Cs, fs, indices, ys, display_times=True):
    ...
    r = (
        hash_to_int([hash(C) for C in Cs] + ys + [kzg_utils.DOMAIN[i] for i in indices])
        % MODULUS
    )
...
def make_verkle_proof(trie, keys, display_times=True):
    ...
    # Nodes sorted
    nodes_sorted_by_index_and_subindex = list(
        map(lambda x: x[1], sorted(nodes_by_index_and_subindex.items()))
    )
    ...
    for node in nodes_sorted_by_index_and_subindex:
        fs.append(
            [
                int.from_bytes(node[i]["hash"], "little") if i in node else 0
                for i in range(WIDTH)
            ]
        )
    ...
    commitments_sorted_by_index_serialized = [
        x["commitment"].compress() for x in nodes_sorted_by_index[1:]
    ]
...
def check_verkle_proof(trie, keys, values, proof, display_times=True):
    ...
    for key, value, depth in zip(keys, values, depths):
        ...
        leaf_values_by_index_and_subindex[
            (verkle_indices[: depth - 1], verkle_indices[depth - 1])
        ] = hash([key, value])
    ...
    commitments_by_index = {
        index: commitment
        for index, commitment in zip(all_indices, commitments_sorted_by_index)
    }
    commitments_by_index_and_subindex = {
        index_and_subindex: commitments_by_index[index_and_subindex[0]]
    ...
    for index_and_subindex in all_indices_and_subindices:
        ...
        if full_subindex in commitments_by_index:
            subhashes_by_index_and_subindex[index_and_subindex] = hash(
                commitments_by_index[full_subindex]
            )
        else:
            subhashes_by_index_and_subindex[index_and_subindex] = (
                leaf_values_by_index_and_subindex[index_and_subindex]
            )
    ...
    ys = list(
        map(
            lambda x: int.from_bytes(x[1], "little"),
            sorted(subhashes_by_index_and_subindex.items()),
        )
    )

    log_time_if_eligible("   Recreated commitment lists", 30, display_times)

    return check_kzg_multiproof(
        Cs, indices, ys, [D_serialized, y, sigma_serialized], display_times
    )
...
if __name__ == "__main__":
    ...
    print(
        "Inserted {0} elements for an average depth of {1:.3f}".format(
            NUMBER_INITIAL_KEYS, average_depth
        ),
        file=sys.stderr,
    )
    ...
    if NUMBER_ADDED_KEYS > 0:
        ...
        print(
            "Additionally inserted {0} elements in {1:.3f} s".format(
                NUMBER_ADDED_KEYS, time_y - time_x
            ),
            file=sys.stderr,
        )
        print(
            "Keys in tree now: {0}, average depth: {1:.3f}".format(
                get_total_depth(root)[1], get_average_depth(root)
            ),
            file=sys.stderr,
        )
